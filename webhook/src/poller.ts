...
// --- Local models (aligns with previous code) ---
type Stop = z.infer<typeof StopDto>;
type Route = {
  id: string;
  shipmentId?: string;
  driverId?: string;
...
type DriverState = {
  ...
  phase: "IDLE" | "ENROUTE" | "AT_STOP" | "COMPLETED";
  lastLat?: number;
  lastLon?: number;
  lastUpdateAt?: string;
  arrivedAt?: string;
...
// --- DB helpers ---
async function upsertRoute(dto: z.infer<typeof RouteDto>) {
  const stops: Stop[] = dto.stops.map((s, i) => ({
    id: `${dto.id}-${i}`,
    type: s.type,
    location: {
      lat: s.location.lat,
...
async function getDriverState(driverId: string): Promise<DriverState | undefined> {
  const res = await ddb.send(
    new GetCommand({ TableName: DRIVER_TABLE, Key: { driverId } })
  );
  return res.Item as DriverState | undefined;
}

async function ensureDriverAssigned(driverId: string, routeId: string) {
  ...
  await ddb.send(
    new UpdateCommand({
      ...
      Key: { driverId },
      UpdateExpression:
        "SET #routeId = :r, #phase = :ph, #currentStopIndex = if_not_exists(#currentStopIndex, :i0), #version = if_not_exists(#version, :v0) + :inc",
...
export const handler: Handler = async () => {
  ...
  const [driversRaw, routesRaw] = await Promise.all([
    getJson<unknown[]>("/drivers"),
    getJson<unknown[]>("/routes"),
  ]);
  ...
  const driversNeedingDetail = new Set<string>();
  for (const r of routes) if (r.driverId) driversNeedingDetail.add(r.driverId);
  ...
  await mapLimit(Array.from(driversNeedingDetail), CONC, async (driverId) => {
    try {
      const detail = await getJson<unknown>(`/drives/${driverId}/routes`);
      const parsed = RouteDto.parse(detail);
      await upsertRoute(parsed);
    } catch (e) {
      // not fatal if endpoint doesn’t exist yet
      console.warn(`No detail for driver ${driverId}:`, (e as Error).message);
    }
  });
  ...
  await mapLimit(routes.filter((r) => r.driverId), CONC, async (r) => {
    if (r.driverId) await ensureDriverAssigned(r.driverId, r.id);
  });
